...
const handlers = {
    ...
    mbway: (name, amount, email, phone) =>
        submitMBway(name, amount, email, phone).then(({ id }) => ({
            intentid: id,
            method: 'mbway',
            site: 'easypay',
        })),
...
export const updatePayment = (notificationId) => {
    return loadMBwayPaymentUpdate(notificationId).then((result) =>
        query(`UPDATE donations SET payed=1, updated_at=NOW() WHERE payment_id="${result.id}"`)
    );
};
...
export const updateStripePayment = (data) => {
    ...
    const paymentId =
        payment_method_details.type === 'multibanco' ? payment_method : payment_intent;
